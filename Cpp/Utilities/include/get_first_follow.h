...
class Expr {
 public:
  ...
  Expr(const Expr& rhs) {
    ...
    p_right_production_ =
        std::make_shared<std::string>(rhs.get_right_production());
  }

  std::string get_left_production() const { return left_production_; }

  std::string get_right_production() const { return *p_right_production_; }
...
class Productions {
 public:
  ...
  Productions(Expr::ExprVec& expressions)
      : expressions_(expressions),
        num_of_expression_(expressions_.size()),
        is_first_set_filled_flag(false) {
  ...
  int get_expr_num() { return num_of_expression_; }
 ...
 private:
  ...
  void find_begin_with_spec_symbol_expr(const string& left_symbol,
  ...
  bool determin_symbol_null(const string& left_symbol, unsigned int pos,
                            bool is_left);
  ...
  void get_first_set_helper(const string& left_symbol,
