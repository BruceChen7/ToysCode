...
int NFA::get_final_status() { return final_status_; }

int NFA::get_status_num() { return status_.size(); }

void NFA::set_final_status(int status) { final_status_ = status; }
...
void NFA::display_nfa() {
  auto it = transition_list_.cbegin();
  for (; it != transition_list_.cend(); it++) {
    std::cout << it->begin_status << "---->" << it->end_status
              << " :  Symbol : " << it->trans_symbol << std::endl;
  }
}

void NFA::set_status(int status) {
  for (int i = 0; i < status; i++) status_.push_back(i);
}
...
NFA concatenate_nfa(NFA first, NFA second) {
  ...
  for (; it != concatenated_trans.cend(); it++) {
    concatenated_nfa.set_transition(it->begin_status, it->end_status,
                                    it->trans_symbol);
  }
  ...
  for (; it != concatenated_trans.cend(); it++) {
    concatenated_nfa.set_transition(it->begin_status + first.get_status_num(),
                                    it->end_status + first.get_status_num(),
                                    it->trans_symbol);
  }

  // set the last status is the final status
  concatenated_nfa.set_final_status(first.get_status_num() +
                                    second.get_status_num());
