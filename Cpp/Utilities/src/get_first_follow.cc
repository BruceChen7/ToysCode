...
bool is_belonged_to_a_set(const string& sym,
                          const Productions::StringVec& set) {
...
void Productions::find_begin_with_spec_symbol_expr(const string& left_symbol,
...

{
  ...
  unsigned int i;
  for (i = 0u; i < length; i++) {
    if (first_set_[i].symbol_name == sym) break;
  }
...
bool Productions::is_belonged_to_first_set(const string& symbol,
                                           StringVec& set) {
...
//
bool Productions::determin_symbol_null(const std::string& left_symbol,
                                       unsigned int pos, bool is_left) {
  if (is_in_null_sets(left_symbol)) return true;
  if (is_a_terminal(left_symbol))
    ...
    for (unsigned int i = 0; i < expressions_.size(); i++) {
      ...
      if (new_left_symbol == left_symbol) {
        ...
        for (unsigned int j = 0; j < right_production.size(); j++) {
          ...
          // the same production and the right part of production hass also
          // left_symbol we can't determin whether if the left_symbol is a
          // nullable symbol by this expression
    ...
    if (is_left == true) nullable_set_.push_back(left_symbol);
    return true;
  }
}
...
void Productions::get_first_set() {
  if (is_first_set_filled_flag) return;
  ...
  if (is_a_terminal(non_terminal))
    ...
    for (const auto& set : first_set_) {
      if (set.symbol_name == non_terminal) {
        ...
        std::cout << std::endl;
        std ::cout << "the num of first set is "
                   << set.first_or_follow_set.size() << std::endl;
      }
    }
  }
}
...
//把它输出到first_set中
void Productions::get_first_set_helper(const std::string& left_production,
  ...
  for (auto& i : pos) {
    ...
    if (num == -1) {
      ...
      for (auto j = 0; j <= num + 1; j++) {
        ...
        if (right_symbol == left_production) break;
        get_first_set_helper(right_symbol, right_first_set);
...
void Productions::get_follow_set() {
  if (!is_first_set_filled()) get_first_set();
...
bool Productions::is_first_set_filled() { return is_first_set_filled_flag; }
