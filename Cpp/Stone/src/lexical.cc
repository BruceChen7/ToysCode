...
// global variables;
const char *g_err_buf[32] = {"All is well", "The Token Is Too Long",
                             "I Do Not know What The Symbol Is,May Be,You "
                             "Should Use Blank Character To Separate Them"};
...
int File::get_file_line_num() const { return line_num_; }
...
Lexical::Lexical(const char *file_name)
    : source_code_file_(std::unique_ptr<File>(new File(file_name))),
      err_code_(0) {
...
void Lexical::parse() {
  ...
  for (const auto &err_msg : err_vec_) ::fprintf(stderr, "%s", err_msg.data());
}
...
size_t Lexical::get_token_num() { return token_list_.size(); }
