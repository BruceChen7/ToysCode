...
toys_skip_list_node* toys_sl_create_node(int level, double score, void* value) {
    toys_skip_list_node* sl_node = TOYS_ALLOC(
        sizeof(*sl_node) + level * sizeof(struct toys_skip_list_level));
...
void toys_sl_free_node(toys_skip_list_node* sl_node) { TOYS_FREE(sl_node); }

// return a random level for the new skip list  Node we are going to create
// the new level is between 1 to TOYS_SKIP_LIST_MAX_LEVEL (include
// TOYS_SKIP_LIST_MAX_LEVEL)
//
int toys_sl_random_level(void) {
    int level = 1;

    while ((random() & 0xFFFF) < (TOYS_SKIP_LIST_P * 0xffff)) level += 1;
    return (level < TOYS_SKIP_LIST_MAX_LEVEL) ? level
                                              : TOYS_SKIP_LIST_MAX_LEVEL;
}
...
toys_skip_list_node* toys_sl_insert(toys_skip_list* sl, double score,
                                    void* value) {
