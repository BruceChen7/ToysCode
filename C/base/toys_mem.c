...
void *toys_mem_calloc(long ncount, long nbytes, const char *file, int line) {
    void *ptr;
    TOYS_ASSERT(ncount > 0,
                "the number of allocated space must be larger than 0");
...
void *toys_mem_resize(void *ptr, long nbytes, const char *file, int line) {
    ...
    TOYS_ASSERT(nbytes >= 0,
                "the size which is reallocated must be larger than 0");

    ptr = realloc(ptr, nbytes);

    if (ptr == NULL)
        fprintf(stderr, "Reallocation Error in file : %s  line : %d", file,
                line);
